library flutter_swipe_card;

import 'dart:math' as Math;
import 'package:flutter/widgets.dart';

enum SwipePosition { None, Left, Right }
enum StackFrom { None, Top, Left, Right, Bottom }
enum StackMovingDirection { Flexible, FromBottomToLeft, FromBottomToRight }

class SwipeItem {

  Widget Function(SwipePosition, double progress) builder;
...
class SwipeCard extends StatefulWidget {
  ...
  final void Function(int, SwipePosition) onSwipe;
  final void Function(int, SwipePosition) onRewind;
  final void Function() onEnd;
  final EdgeInsetsGeometry padding;
  final bool avoidSwipingRight;
  final bool avoidSwipingLeft;
  final StackMovingDirection movingDirection;
  ...
  SwipeCard({
    ...
    this.onSwipe,
    this.onRewind,
    this.avoidSwipingRight = false,
    this.avoidSwipingLeft = false,
    this.padding = const EdgeInsets.symmetric(vertical: 20, horizontal: 25),
    this.movingDirection = StackMovingDirection.Flexible,
  }) :
...
class SwipeCardState extends State<SwipeCard> with SingleTickerProviderStateMixin {
  ...
  SwipePosition _currentItemPosition = SwipePosition.None;
  ...
  void initState() {
    ...
    _animationController.addListener(() {
      if (_animationController.status == AnimationStatus.forward) {
        ...
        _currentItemPosition = (_left.toInt() == 0) ? SwipePosition.None : (_left < 0) ? SwipePosition.Left : SwipePosition.Right;
    ...
    _animationController.addStatusListener((AnimationStatus animationStatus) {
      if (animationStatus == AnimationStatus.completed) {
        ...
        _angle = 0;
        _progress = 0;
        _currentItemPosition = SwipePosition.None;
        _animationType = 0;
        setState((){});
  ...
  Widget _item(BoxConstraints constraints, int index) {
        ...
        child: Positioned(
            ...
            child: Transform.scale(
                ...
                child: Container(
                    ...
                    child: widget.children[index].builder(SwipePosition.None, 0)
                )
            )
        ),
      );
    ...
    if (widget.maxAngle > 0 && _animationController.status != AnimationStatus.forward) {
      ...
      if (widget.movingDirection == StackMovingDirection.FromBottomToLeft) {
        _angle = _angle * -1;
      } else if (widget.movingDirection == StackMovingDirection.FromBottomToRight) {
        _angle = _angle * 1;
      } else {
  ...
  bool get _shouldBackToFirstPosition => (widget.avoidSwipingRight && _left > 0) || (widget.avoidSwipingLeft && _left < 0);

  void _onPanEnd(_) {
    setState((){});
    if (_progress < widget.threshold || _shouldBackToFirstPosition) {
      _goFirstPosition();
    } else {
      _animationType = 1;
